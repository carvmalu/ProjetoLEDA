...
echo "Rodando 1000000 testes para pegar a média"
for i in {1..1000000}; do
    if [ $((i % 50000)) -eq 0 ]; then
        echo "Execução $i de 1000000..."
    fi
    ./main_haskell 10000 30000 50000 100000 2>/dev/null
    busca=$(grep "^Haskell_dataVector,10000,busca," Resultados/Haskell/resultados_10000.csv 2>/dev/null | cut -d',' -f4)
    [ ! -z "$busca" ] && soma_busca_10k=$(echo "$soma_busca_10k + $busca" | bc)
    add=$(grep "^Haskell_dataVector,10000,adicaoInicio," Resultados/Haskell/resultados_10000.csv 2>/dev/null | cut -d',' -f4)
    [ ! -z "$add" ] && soma_add_inicio_10k=$(echo "$soma_add_inicio_10k + $add" | bc)
    rem=$(grep "^Haskell_dataVector,10000,remocaoIndice," Resultados/Haskell/resultados_10000.csv 2>/dev/null | cut -d',' -f4)
    [ ! -z "$rem" ] && soma_rem_indice_10k=$(echo "$soma_rem_indice_10k + $rem" | bc)
    rval=$(grep "^Haskell_dataVector,10000,remocaoValor," Resultados/Haskell/resultados_10000.csv 2>/dev/null | cut -d',' -f4)
    [ ! -z "$rval" ] && soma_rem_valor_10k=$(echo "$soma_rem_valor_10k + $rval" | bc)
    busca=$(grep "^Haskell_dataVector,30000,busca," Resultados/Haskell/resultados_30000.csv 2>/dev/null | cut -d',' -f4)
    [ ! -z "$busca" ] && soma_busca_30k=$(echo "$soma_busca_30k + $busca" | bc)
    add=$(grep "^Haskell_dataVector,30000,adicaoInicio," Resultados/Haskell/resultados_30000.csv 2>/dev/null | cut -d',' -f4)
    [ ! -z "$add" ] && soma_add_inicio_30k=$(echo "$soma_add_inicio_30k + $add" | bc)
    rem=$(grep "^Haskell_dataVector,30000,remocaoIndice," Resultados/Haskell/resultados_30000.csv 2>/dev/null | cut -d',' -f4)
    [ ! -z "$rem" ] && soma_rem_indice_30k=$(echo "$soma_rem_indice_30k + $rem" | bc)
    rval=$(grep "^Haskell_dataVector,30000,remocaoValor," Resultados/Haskell/resultados_30000.csv 2>/dev/null | cut -d',' -f4)
    [ ! -z "$rval" ] && soma_rem_valor_30k=$(echo "$soma_rem_valor_30k + $rval" | bc)
    busca=$(grep "^Haskell_dataVector,50000,busca," Resultados/Haskell/resultados_50000.csv 2>/dev/null | cut -d',' -f4)
    [ ! -z "$busca" ] && soma_busca_50k=$(echo "$soma_busca_50k + $busca" | bc)
    add=$(grep "^Haskell_dataVector,50000,adicaoInicio," Resultados/Haskell/resultados_50000.csv 2>/dev/null | cut -d',' -f4)
    [ ! -z "$add" ] && soma_add_inicio_50k=$(echo "$soma_add_inicio_50k + $add" | bc)
    rem=$(grep "^Haskell_dataVector,50000,remocaoIndice," Resultados/Haskell/resultados_50000.csv 2>/dev/null | cut -d',' -f4)
    [ ! -z "$rem" ] && soma_rem_indice_50k=$(echo "$soma_rem_indice_50k + $rem" | bc)
    rval=$(grep "^Haskell_dataVector,50000,remocaoValor," Resultados/Haskell/resultados_50000.csv 2>/dev/null | cut -d',' -f4)
    [ ! -z "$rval" ] && soma_rem_valor_50k=$(echo "$soma_rem_valor_50k + $rval" | bc)
    busca=$(grep "^Haskell_dataVector,100000,busca," Resultados/Haskell/resultados_100000.csv 2>/dev/null | cut -d',' -f4)
    [ ! -z "$busca" ] && soma_busca_100k=$(echo "$soma_busca_100k + $busca" | bc)
    add=$(grep "^Haskell_dataVector,100000,adicaoInicio," Resultados/Haskell/resultados_100000.csv 2>/dev/null | cut -d',' -f4)
    [ ! -z "$add" ] && soma_add_inicio_100k=$(echo "$soma_add_inicio_100k + $add" | bc)
    rem=$(grep "^Haskell_dataVector,100000,remocaoIndice," Resultados/Haskell/resultados_100000.csv 2>/dev/null | cut -d',' -f4)
    [ ! -z "$rem" ] && soma_rem_indice_100k=$(echo "$soma_rem_indice_100k + $rem" | bc)
    rval=$(grep "^Haskell_dataVector,100000,remocaoValor," Resultados/Haskell/resultados_100000.csv 2>/dev/null | cut -d',' -f4)
...
media_add_inicio_10k=$(format_number "$media_add_inicio_10k")
media_rem_indice_10k=$(format_number "$media_rem_indice_10k")
media_rem_valor_10k=$(format_number "$media_rem_valor_10k")
media_busca_30k=$(format_number "$media_busca_30k")
media_add_inicio_30k=$(format_number "$media_add_inicio_30k")
media_rem_indice_30k=$(format_number "$media_rem_indice_30k")
media_rem_valor_30k=$(format_number "$media_rem_valor_30k")
media_busca_50k=$(format_number "$media_busca_50k")
media_add_inicio_50k=$(format_number "$media_add_inicio_50k")
media_rem_indice_50k=$(format_number "$media_rem_indice_50k")
media_rem_valor_50k=$(format_number "$media_rem_valor_50k")
media_busca_100k=$(format_number "$media_busca_100k")
media_add_inicio_100k=$(format_number "$media_add_inicio_100k")
media_rem_indice_100k=$(format_number "$media_rem_indice_100k")
media_rem_valor_100k=$(format_number "$media_rem_valor_100k")
